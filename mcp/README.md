...
| API_URL | URL of the upstream Todo API | `https://todo-api-ping-federate.<ENV>.workers.dev` |
...
This verification step observes how the MCP server performs token exchange to acquire a new token audienced for the upstream API. Using the `who_am_I` and `peek_api_tokens` tools, we will compare the two tokens.
